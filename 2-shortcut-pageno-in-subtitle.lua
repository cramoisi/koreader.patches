...
function FileManager:updateTitleBarPath(path)
	...
	-- Use shortcut name if available, otherwise derive folder name from path
	local label
	if self.folder_shortcuts:hasFolderShortcut(path) then
		local shortcut = self.folder_shortcuts.folder_shortcuts[path]
		label = shortcut and BD.directory(shortcut.text) or BD.directory(filemanagerutil.abbreviate(path))
	else
		local text = BD.directory(filemanagerutil.abbreviate(path))
		local dir_name = util.splitToArray(text, "/")
		label = dir_name[#dir_name]
	end

	local subtitle = total_pages ~= 0 and
			label .. " - " .. T(_("Page %1 of %2"), current_page, total_pages) or
			label
