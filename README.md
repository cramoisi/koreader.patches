# koreader.patches
some koreader patches made for my own usage. If prefix PT they need Project title plugin (https://github.com/joshuacant/ProjectTitle)

## Plugins for Project:Title

### 2-pt-list-datafromfilename
Project title : Don't use metadata but filename to display author, series and title in project title list views

### 2-pt-list-disable-prbar
 Project title : Disable progress bars for finished and new books in PT list views but keep the marker

 ### 2-pt-list-localize-status
 Project title : change the language of books status (new, reading, on hold) without changing all koreader UI language

## Plugins for Koreader

 ### 2-reader-filename-header-print-edition
A modified version of joshuacant 2-reader-header-print-edition (https://github.com/joshuacant/KOReader.patches/blob/main/2-reader-header-print-edition.lua) to use the filename instead of metadata on the odd pages

### 2-header-filename
Display the filename (centered) and the page number (to the right) at the top of each page of a epub file

### 2-ignore-rakuyomi-history
Hides everything read within Rakuyomi from the koreader's history as filenames are not human readable
